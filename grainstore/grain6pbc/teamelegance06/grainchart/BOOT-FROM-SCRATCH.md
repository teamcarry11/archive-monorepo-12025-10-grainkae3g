...
git clone https://github.com/grain6pbc/babashka
cd babashka
...
clone_grainstore() {
  ~/.local/bin/git clone https://github.com/grain6pbc/grainstore
  # Now we have ALL our forks, ALL our code
}

# Step 4: Build dependencies FROM OUR FORKS
build_babashka() {
  cd grainstore/grain6pbc/dependencies/babashka
  # Build from source using OUR fork
  # Install to ~/.local/bin
}

build_clojure() {
  cd grainstore/grain6pbc/dependencies/clojure
  # Build from source
  # Install to ~/.local/bin
}
...
jobs:
  test-alpine:
    ...
    steps:
      - name: Test bootstrap
        run: |
          wget https://raw.githubusercontent.com/grain6pbc/grainstore/main/bootstrap.sh
          sh bootstrap.sh
...
**Core Tools**:
- **Babashka** → grain6pbc/babashka (Clojure scripting)
- **Clojure** → grain6pbc/clojure (Core language)
- **tools.deps** → grain6pbc/tools.deps.alpha (Dependency management)

**System Tools**:
- **Git** → grain6pbc/git (Version control)
- **Zsh** → grain6pbc/zsh (Shell)
- **s6** → grain6pbc/s6 (Init system)

**OS Components**:
- **Alpine APKs** → grain6pbc/alpine-packages (Mirror)
- **NixOS pkgs** → grain6pbc/nixpkgs (Fork)
- **musl libc** → grain6pbc/musl (C library)

**Astrological**:
- **Swiss Ephemeris** → grain6pbc/swisseph (Astronomical calculations)
...
{:dependencies
 [{:name "babashka"
   :fork "https://github.com/grain6pbc/babashka"
  ...
  {:name "clojure"
   :fork "https://github.com/grain6pbc/clojure"
...
- [ ] Fork Babashka to grain6pbc
- [ ] Fork Clojure to grain6pbc  
- [ ] Fork Git to grain6pbc
...
- **teamrebel10/graintime** - Already uses `gt` command (installed to ~/.local/bin)

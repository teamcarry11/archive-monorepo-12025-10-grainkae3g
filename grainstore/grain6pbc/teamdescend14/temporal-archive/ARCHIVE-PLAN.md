...
**Team**: teamdescend14 (Ketu, descent wisdom, documentation)
...
**Structure**:
```
teamdescend14/temporal-archive/
├── 12025-10-26/
...
git log --all --format='%H %ci' | while read hash date time tz; do
  ...
  git show --name-only --format="" $hash | grep '\.md$' | while read file; do
    ...
    cp "$file" "teamdescend14/temporal-archive/${folder_date}/${timestamp}--$(basename $file)"
  done
done
```
...
git log --all --name-only --format='%H %ci' | grep '\.md$' | while read ...; do
  # Copy to teamdescend14/temporal-archive/external/{repo}/{date}/
done
```
...
**File**: `teamdescend14/temporal-archive/scripts/archive-session.clj`
...
3. Consolidate into grainkae3g or grain6pbc
...
- ✅ All files in one place (teamdescend14/temporal-archive/)

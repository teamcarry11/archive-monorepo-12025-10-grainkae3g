(ns graindaemon.github-description-sync
  "Graindaemon: GitHub Repository Description Sync
  
  Automatically updates GitHub repository description with grainsite URL
  whenever a new grainpath branch is created or updated."
...
(defn get-current-grainpath []
  ...
  (try
    (let [result (shell {:out :string} "git" "branch" "--show-current")
      ...
      (if (str/starts-with? branch-name "grain")
        branch-name
        nil))
    (catch Exception e
...
(defn -main [& args]
  "Main entry point for graindaemon github-description-sync"
  (sync-github-description))

;; Export for use in other namespaces
(def sync-github-description sync-github-description)

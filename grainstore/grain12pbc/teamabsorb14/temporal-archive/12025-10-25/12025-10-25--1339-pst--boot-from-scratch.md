...
git clone https://github.com/grain12pbc/babashka
cd babashka
...
clone_grainstore() {
  ~/.local/bin/git clone https://github.com/grain12pbc/grainstore
  # Now we have ALL our forks, ALL our code
}

# Step 4: Build dependencies FROM OUR FORKS
build_babashka() {
  cd grainstore/grain12pbc/dependencies/babashka
  # Build from source using OUR fork
  # Install to ~/.local/bin
}

build_clojure() {
  cd grainstore/grain12pbc/dependencies/clojure
  # Build from source
  # Install to ~/.local/bin
}
...
jobs:
  test-alpine:
    ...
    steps:
      - name: Test bootstrap
        run: |
          wget https://raw.githubusercontent.com/grain12pbc/grainstore/main/bootstrap.sh
          sh bootstrap.sh
...
**Core Tools**:
- **Babashka** → grain12pbc/babashka (Clojure scripting)
- **Clojure** → grain12pbc/clojure (Core language)
- **tools.deps** → grain12pbc/tools.deps.alpha (Dependency management)

**System Tools**:
- **Git** → grain12pbc/git (Version control)
- **Zsh** → grain12pbc/zsh (Shell)
- **s6** → grain12pbc/s6 (Init system)

**OS Components**:
- **Alpine APKs** → grain12pbc/alpine-packages (Mirror)
- **NixOS pkgs** → grain12pbc/nixpkgs (Fork)
- **musl libc** → grain12pbc/musl (C library)

**Astrological**:
- **Swiss Ephemeris** → grain12pbc/swisseph (Astronomical calculations)
...
{:dependencies
 [{:name "babashka"
   :fork "https://github.com/grain12pbc/babashka"
  ...
  {:name "clojure"
   :fork "https://github.com/grain12pbc/clojure"
...
- [ ] Fork Babashka to grain12pbc
- [ ] Fork Clojure to grain12pbc  
- [ ] Fork Git to grain12pbc

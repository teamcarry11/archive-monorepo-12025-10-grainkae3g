<script>
  ...
  // Props: φ-data from Steel backend (JSON)
  ...
  // ============================================================================
  // ⚡ TWEENED VALUES (Smooth Animations!)
  // ============================================================================
  ...
  // Think of it like a smooth transition instead of a sudden jump! 🌊
  ...
  // When component mounts, load the φ-data generated by Steel!
  onMount(async () => {
    if (!phiData) {
      try {
        const response = await fetch('/phi-data.json');
        phiData = await response.json();
        console.log('🌀 φ-data loaded!', phiData);
      } catch (error) {
        console.error('❌ Failed to load φ-data:', error);
  ...
  // Generate φ-coordinates (client-side fallback)
  ...
  // ============================================================================
  // 🌀 TAP HANDLERS (The Core Interaction!)
  // ============================================================================

  // Tap anywhere = spiral inward by φ!
  // This is the main interaction - NO SCROLLING, just tapping! 🌊
  function spiralInward(event) {
    ...
    // Each level is 1/φ times the previous (61.8% size)
  ...
  // Tap back button = spiral outward by φ!
  ...
  // Get φ-coordinates for current level
...
<style>
  ...
  .graincard-container {
    width: 100vw;
    height: 100vh;
    overflow: hidden; /* NO SCROLLING! This is key! 🌊 */
  ...
  /* φ-markers show the golden ratio points */
  .phi-marker {
    position: absolute;
    width: 12px;
    height: 12px;
...
<div class="graincard-container" on:click={spiralInward}>
  ...
  <div 
    ...
    style="
      ...
      opacity: {$opacity};
    ">
    {@html currentSectionData.content || graincardContent || 'Loading φ-vortex...'}
  </div>

  <!-- Seed indicator (when at deepest level) -->
  {#if atSeed}
    <div class="seed-indicator">⚡</div>
  {/if}
  ...
  <div class="instruction">
    ...
    {:else if atSeed}
      ...
      <br>
      ← Tap back to spiral out
    {:else}
      ⚡ TAP to go deeper<br>
      ← or BACK to return<br>
      <br>

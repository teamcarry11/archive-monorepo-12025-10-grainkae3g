...
The `grainsource-personalize` module automates the process of converting a `grain12pbc` template repository into a personal `grain{devname}{module}` repository by:
...
### Templates (grain12pbc)
```
grain12pbc/graincourse              ← Template with personal/ gitignored
grain12pbc/graintime                ← Template with personal/ gitignored
grain12pbc/grainzsh                 ← Template with personal/ gitignored
grain12pbc/grainenvvars             ← Template with personal/ gitignored
```
...
Following the `grain12pbc/graindevname` specification:
...
git clone https://github.com/grain12pbc/graincourse.git
cd graincourse
```
...
# - Update all references from grain12pbc to kae3g
# - Set up git remotes
```
...
bb personalize jen3g time ~/grain/grain12pbc/graintime
...
```gitignore
# grain12pbc/graincourse/.gitignore
...
### File Renames
- `README.md` → Update all `grain12pbc` to `kae3g`
...
### URL Updates
```
https://github.com/grain12pbc/graincourse
→ https://github.com/kae3g/grainkae3gcourse

https://codeberg.org/grain12pbc/graincourse
...
```clojure
;; Before
(ns grain12pbc.graincourse.core ...)
...
```bash
# From grain12pbc clone directory
...
# ✓ All references updated: grain12pbc → kae3g
# ✓ Git remotes configured
...
1. **Templates are Universal**: `grain12pbc` repos work for everyone
...
```
grain12pbc → grain{devname}{module}
template → personal
universal → unique
shared → versioned
```

...
;; Manages grainbranches across all grain12pbc repositories

(require '[babashka.process :refer [shell]]
         '[clojure.string :as str])

(def grain12pbc-repos
  "List of grain12pbc repositories to manage"
  ["grain12pbc/grain6"
   "grain12pbc/grainkae3g"
   "grain12pbc/graincontacts"
   "grain12pbc/humble-stack"
   "grain12pbc/graindaemon"])
...
(defn create-grainbranch-name [project-name]
  ...
  (let [graintime (get-current-graintime)
        clean-project (str/replace project-name #"grain12pbc/" "")]
...
(defn get-grainsite-url [repo-name grainbranch-name]
  ...
  (let [clean-repo (str/replace repo-name #"grain12pbc/" "")
        subdomain (case clean-repo
                    "grain6" "grain12pbc.com"
                    "grainkae3g" "kae3g.grain12pbc.com"
                    "graincontacts" "contacts.grain12pbc.com"
                    "humble-stack" "ui.grain12pbc.com"
                    "graindaemon" "daemon.grain12pbc.com"
                    (str clean-repo ".grain12pbc.com"))]
  ...
  (let [grainbranch-name (create-grainbranch-name repo-name)
    ...
    (try
      ...
      (shell "gh" "api" (str "repos/" repo-name) "--method" "PATCH"
             "--field" (str "description=🌾 Grain6pbc " (str/replace repo-name #"grain12pbc/" "") ": " grainbranch-name " | Live Site: " grainsite-url " | Session 780 Complete"))

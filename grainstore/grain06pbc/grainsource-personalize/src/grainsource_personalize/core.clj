(ns grainsource-personalize.core
  ...
  Converts grain06pbc templates into grain{devname}{module} personal repos by:
...
(defn ungitignore-personal
  "Remove personal/ from .gitignore file
   ...
   In template repos (grain06pbc), personal/ is gitignored.
...
(defn update-references
  "Update all grain06pbc references to grain{devname}{module}
   
   Updates:
   - grain06pbc → devname (in text)
  ...
  (let [old-name (str "grain" module)
    ...
    {:old-name old-name
     ...
     :replacements
     {;; Repository names
      (str "grain06pbc/" old-name) (str new-org "/" new-name)
      (str "grain06pbc." old-name) (str new-org "." new-name)
      
      ;; URLs
      (str "github.com/grain06pbc/" old-name) (str "github.com/" new-org "/" new-name)
      (str "codeberg.org/grain06pbc/" old-name) (str "codeberg.org/" new-org "/" new-name)
      
      ;; Namespaces (if applicable)
      (str "grain06pbc." old-name ".") (str new-name ".")
...
(defn verify-personalization
  "Verify that a repository has been properly personalized
   ...
   - All grain06pbc references updated
  ...
  (let [gitignore-path (str repo-dir "/.gitignore")
        ...
        readme-content (when (.exists (io/file readme-path))
                        (slurp readme-path))
        has-grain06pbc-refs? (and readme-content
                               (str/includes? readme-content "grain06pbc"))
        ...
        issues (cond-> []
                 ...
                 has-grain06pbc-refs?
                 (conj "grain06pbc references still present (should be updated)")
    ...
    {:valid (empty? issues)
     ...
     :checks {:personal-gitignored? has-personal-gitignored?
              :grain06pbc-refs? has-grain06pbc-refs?

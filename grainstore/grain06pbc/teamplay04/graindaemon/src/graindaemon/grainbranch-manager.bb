...
;; Manages grainbranches across all grain06pbc repositories

(require '[babashka.process :refer [shell]]
         '[clojure.string :as str])

(def grain06pbc-repos
  "List of grain06pbc repositories to manage"
  ["grain06pbc/grain6"
   "grain06pbc/grainkae3g"
   "grain06pbc/graincontacts"
   "grain06pbc/humble-stack"
   "grain06pbc/graindaemon"])
...
(defn create-grainbranch-name [project-name]
  ...
  (let [graintime (get-current-graintime)
        clean-project (str/replace project-name #"grain06pbc/" "")]
...
(defn get-grainsite-url [repo-name grainbranch-name]
  ...
  (let [clean-repo (str/replace repo-name #"grain06pbc/" "")
        subdomain (case clean-repo
                    "grain6" "grain06pbc.com"
                    "grainkae3g" "kae3g.grain06pbc.com"
                    "graincontacts" "contacts.grain06pbc.com"
                    "humble-stack" "ui.grain06pbc.com"
                    "graindaemon" "daemon.grain06pbc.com"
                    (str clean-repo ".grain06pbc.com"))]
  ...
  (let [grainbranch-name (create-grainbranch-name repo-name)
    ...
    (try
      ...
      (shell "gh" "api" (str "repos/" repo-name) "--method" "PATCH"
             "--field" (str "description=🌾 Grain6pbc " (str/replace repo-name #"grain06pbc/" "") ": " grainbranch-name " | Live Site: " grainsite-url " | Session 780 Complete"))
